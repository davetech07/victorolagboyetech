import { portfolioData, skills, projects, contactLinks, contactForm } from "./data.js";
import { HeroSection, AboutSection, ProjectsSection, ContactSection } from "./components/index.js";
...
function setupContactForm() {
    ...
    const fields = {
        name: {
            ...
            validate: (value) => value.trim().length >= 2 || "Please enter at least 2 characters."
        },
        email: {
        ...
        message: {
            ...
            validate: (value) => value.trim().length >= 20 || "Share a little more detail (min 20 characters)."
        }
    };

    function renderFieldError(name, message) {
        const field = fields[name];
        if (!field || !(field.error instanceof HTMLElement) || !(field.input instanceof HTMLElement)) {
            return;
        }
    ...
    form.addEventListener("submit", async (event) => {
        ...
        if (contactForm.endpoint.includes("your-form-id")) {
            status.textContent = "Add your Formspree form ID in data.js to enable live submission.";
            status.classList.add("error");
            return;
        }
